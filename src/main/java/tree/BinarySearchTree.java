 ...
 * 		（最坏情况）当先后插入的关键字有序时，构成的二叉排序树蜕变为【单枝树】。查找性能为O(n)
 * 		（最好情况）二叉排序树的形态和折半查找的判定树相同，其  平均查找长度和log2(n)成正比
 *
 *
 * 插入、删除性能：
...
public class BinarySearchTree {
    ...
    public void nrInOrderTraverse(){
        ...
        while(node != null || !stack.isEmpty()){
            while(node != null){ //一直入左结点，直到为null
                stack.push(node);
                node = node.getLeft();
            }
            node = stack.pop();
            System.out.println(node.getValue());//访问结点
            node = node.getRight();
        }
    }
    ...
    public static void main(String[] args){
        ...
        /**构建的二叉树没有相同元素*/
        int[] num = {4,7,2,1,10,6,9,3,8,11,2,0,-2}; //4是根节点
